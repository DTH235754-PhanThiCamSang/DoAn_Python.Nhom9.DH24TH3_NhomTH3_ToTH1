...
class FrmSanPham(tk.Toplevel):
    def __init__(self, parent, connect_db_func=connect_db):
        ...
        tree.column("masp", width=50, anchor="center")
        tree.column("tensp", width=150)
        tree.column("loaisp", width=150)
        tree.column("dvt", width=60)
        tree.column("giaban", width=50)
        tree.column("hsd", width=150)
        tree.column("mancc", width=80, anchor="center")
        ...
        def xoa_sp():
            ...
            if not selected:
                ...
                return
            masp = tree.item(selected)["values"][0]
            conn = connect_db()
            cur = conn.cursor()
        ...
        tk.Button(frame_btn, text="Thêm", width=8, command=them_sp).grid(row=0, column=0, padx=5)
        tk.Button(frame_btn, text="Lưu", width=8, command=luu_sp).grid(row=0, column=1, padx=5)
        tk.Button(frame_btn, text="Sửa", width=8, command=sua_sp).grid(row=0, column=2, padx=5)
        tk.Button(frame_btn, text="Hủy", width=8, command=xoatext).grid(row=0, column=3, padx=5)
        tk.Button(frame_btn, text="Xóa", width=8, command=xoa_sp).grid(row=0, column=4, padx=5)
        tk.Button(frame_btn, text="Đóng", width=8, command=self.destroy).grid(row=0, column=5, padx=5)

        tk.Button(frame_info, text="Tìm kiếm", width=8, command=tim_kiem).grid(row=2, column=5, padx=5, pady=5, sticky="w")
        tk.Button(frame_info, text="Tải lại", width=8, command=load_data).grid(row=3, column=5, padx=5, pady=5, sticky="w")
