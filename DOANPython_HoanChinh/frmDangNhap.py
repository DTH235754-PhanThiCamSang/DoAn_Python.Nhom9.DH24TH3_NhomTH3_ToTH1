...
import hashlib
from connectdb import connect_db  # import đúng từ connectdb.py
...
class DangNhap(tk.Toplevel):
    def __init__(self, parent=None, on_success=None):
        ...
        self.TaoUI()
        self.bind("<Return>", lambda e: self.dangnhap())
    ...
    def ktrataikhoan(self, username: str, password: str):
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể truy vấn CSDL.\n{e}")
            return False, None
        ...
        ok = (password == db_pwd_hash) if PLAINTEXT_PASSWORD_IN_DB else (sha256(password) == db_pwd_hash)
    ...
    def cauhoibaomat(self, username: str):
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể lấy câu hỏi bảo mật.\n{e}")
            return None
    ...
    def xacminhcautraloi(self, username: str, answer: str) -> bool:
        try:
            ...
            cur.close()
            conn.close()
            return sha256(answer.strip()) == row[0] if row else False
        except Exception as e:
            messagebox.showerror("Lỗi", f"Lỗi xác minh trả lời.\n{e}")
            return False
    ...
    def capnhatmatkhau(self, username: str, new_password: str):
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể cập nhật mật khẩu.\n{e}")
            return False
    ...
    def quenmatkhau(self):
        ...
        if not answer or not self.xacminhcautraloi(username, answer):
            messagebox.showerror("Sai", "Câu trả lời bảo mật không đúng.")
            return
